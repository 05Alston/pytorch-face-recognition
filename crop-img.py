...
for im in list_imgs:
    ...
    if img_cropped_list is not None:
        ...
        for i, prob in enumerate(prob_list):
            if prob>0.90:
                crop = Image.open(os.path.join(BASE_DIR, im))
                ...
                crop = crop.crop((box[0], box[1], box[2], box[3]))
                crop.save(f"InferedPeople/{im.split('.')[0]}-{ind + 1}.png")
                ind += 1 
                # img = cv2.rectangle(img, (box[0],box[1]) , (box[2],box[3]), (0,0,255), 2)

    # image = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    # cv2.imwrite(f"FinalImage/{im.split('.')[0]}.png", image)